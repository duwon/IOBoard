...
void Uart_Send (int8_t no, uint8_t *txData, uint8_t dataLen);
...
//---------------------------------------------------------------------------
int Uart_Receive (uartFIFO_TypeDef *buffer, uint8_t *ch)
{
	if (getByteFromBuffer(buffer, ch) == SUCCESS) return 1;
	return 0;
}
...
void Uart_Send (int8_t no, uint8_t *txData, uint8_t dataLen)
{
	switch (no)
		{
		case RS_232  : HAL_UART_Transmit(&huart1, txData, dataLen, 0xFFFF);  break;
		case RS_485  : HAL_UART_Transmit(&huart2, txData, dataLen, 0xFFFF);  break;
		case RS_Ras  : HAL_UART_Transmit(&huart4, txData, dataLen, 0xFFFF);  break;
		}		
}
