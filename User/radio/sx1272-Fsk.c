...
uint16_t SX1272FskGetRxPacket( void *buffer, uint16_t size )
{
...
uint32_t SX1272FskProcess( void )
{
    ...
    case RF_STATE_RX_INIT:
        ...
        Preamble2SyncTimer = RxTimeoutTimer = TimerGetCurrentTick( );
    ...
    case RF_STATE_RX_SYNC:
        if( ( DIO4 == 1 ) && ( PreambleDetected == false ) )// Preamble
        {
            ...
            Preamble2SyncTimer = TimerGetCurrentTick( );
        }
        ...
        if( ( SyncWordDetected == false ) && ( PreambleDetected == true ) && ( ( TimerGetCurrentTick( ) - Preamble2SyncTimer ) > Preamble2SyncTimeout ) )
        {
        ...
        if( ( SyncWordDetected == false ) &&
            ( PreambleDetected == false ) &&
            ( PacketDetected == false ) &&
            ( ( TimerGetCurrentTick( ) - RxTimeoutTimer ) > PacketTimeout ) )
        {
    ...
    case RF_STATE_RX_RUNNING:
        ...
        // Packet timeout
        if( ( PacketDetected == false ) && ( ( TimerGetCurrentTick( ) - RxTimeoutTimer ) > PacketTimeout ) )
        {
            RFState = RF_STATE_RX_TIMEOUT;
        }
        break;
    ...
    case RF_STATE_TX_READY_WAIT:
        if( DIO3 == 1 )    // TxReady
        {
            ...
            TxTimeoutTimer = TimerGetCurrentTick( );
    ...
    case RF_STATE_TX_RUNNING:
        ...
        if( ( TimerGetCurrentTick( ) - TxTimeoutTimer ) > 1000 )
        {
