...
void SX1276Init(uint32_t Fre, uint8_t Band, uint8_t Sfactor, uint8_t Pwr)
{
...
//    SX1276FskInit( );
    ...
    SX1276LoRaInit(Fre, Band, Sfactor, Pwr);
...
uint16_t SX1276GetRxPacket( void *buffer, uint16_t size )
{
    uint16_t ret;

    if( LoRaOn == false )
    {
//        ret = SX1276FskGetRxPacket( buffer, size );
    }
    else
    {
        ret = SX1276LoRaGetRxPacket( buffer, size );
    }
    return (ret);
}
...
//	GPIO_WriteBit(GPIOD, GPIO_Pin_6, 1);		// tx on
