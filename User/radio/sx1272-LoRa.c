...
uint16_t SX1272LoRaGetRxPacket( void *buffer, uint16_t size )
{
...
uint32_t SX1272LoRaProcess( void )
{
    ...
    case RFLR_STATE_TX_INIT:
        ...
        SX1272WriteBuffer( REG_LR_DIOMAPPING1, &SX1272LR->RegDioMapping1, 2 );
        //printf("write RegDioMapping1: 0x%x\n", SX1272LR->RegDioMapping1);
        uint8_t tmp=0, tmp2=0;
        SX1272Read(REG_LR_DIOMAPPING1, &tmp);
        //printf("read  RegDioMapping1: 0x%x\n",  tmp);
    ...
    case RFLR_STATE_TX_RUNNING:
        SX1272Read(REG_LR_IRQFLAGS, &tmp);
        SX1272Read(REG_LR_DIOMAPPING1, &tmp2);
        if( DIO0 == 1 ) // TxDone
        {
            // Clear Irq
